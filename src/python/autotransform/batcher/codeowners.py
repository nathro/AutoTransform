...
class CodeownersBatcher(Batcher):
    ...
    def batch(self, items: Sequence[Item]) -> List[Batch]:
        ...
        batches += self.create_batches(team_owners, "team_reviewers")

        # Add batches based on individual owners
        batches += self.create_batches(individual_owners, "reviewers")

        # Add unowned batch
        if no_owners:
            batches += self.create_batches({"unowned": no_owners}, None)

        return batches

    def create_batches(
        self, owners: Dict[str, List[Item]], metadata_key: Optional[str]
    ) -> List[Batch]:
        """Create batches from the given owners and items.

        Args:
            owners (Dict[str, List[Item]]): A dictionary mapping owners to their items.
            metadata_key (Optional[str]): The key to use for the owner in the metadata.
                                          If None, no owner is added to the metadata.

        Returns:
            List[Batch]: A list of batches created from the owners and items.
        """
        batches: List[Batch] = []
        for owner, items in owners.items():
            ...
            for i, chunk_items in enumerate(item_chunks, start=1):
                ...
                if self.metadata is not None:
                    ...
                    if metadata_key:
                        metadata.setdefault(metadata_key, []).append(owner)
