...
class CodeownersBatcher(Batcher):
    ...
    def _create_batches(
        self, owners: Dict[str, List[Item]], reviewer_key: Optional[str] = None
    ) -> List[Batch]:
        batches = []
        for owner, items in owners.items():
            if self.max_batch_size is not None and len(items) > self.max_batch_size:
                num_chunks = math.ceil(len(items) / self.max_batch_size)
            ...
            for i, chunk_items in enumerate(item_chunks, start=1):
                ...
                if reviewer_key:
                    metadata.setdefault(reviewer_key, []).append(owner)
