...
class CodeownersBatcher(Batcher):
    ...
    def _create_batches(self, owners: Dict[str, List[Item]], metadata_key: Optional[str]) -> List[Batch]:
        batches: List[Batch] = []
        for owner, items in owners.items():
            ...
            for i, chunk_items in enumerate(item_chunks, start=1):
                title = f"[{i}/{num_chunks}]{self.prefix} {owner}" if num_chunks > 1 else f"{self.prefix} {owner}"
