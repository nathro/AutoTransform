...
class CodeownersBatcher(Batcher):
    ...
    def batch(self, items: Sequence[Item]) -> List[Batch]:
        ...
        batches += self._create_batches(team_owners, "team_reviewers")

        # Add batches based on individual owners
        batches += self._create_batches(individual_owners, "reviewers")

        # Add unowned batch
        if no_owners:
            batches += self._create_batches({"unowned": no_owners}, None)
    ...
    def _create_batches(
        ...
        for owner, items in owners.items():
            item_chunks = self._chunk_items(items)
            for i, chunk_items in enumerate(item_chunks, start=1):
                title = (
                    f"[{i}/{len(item_chunks)}]{self.prefix} {owner}"
    ...
    def _chunk_items(self, items: List[Item]) -> List[List[Item]]:
        if self.max_batch_size is not None and len(items) > self.max_batch_size:
            chunk_size = math.ceil(len(items) / self.max_batch_size)
