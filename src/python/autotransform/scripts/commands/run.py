...
def run_command_main(args: Namespace) -> None:
    ...
    if args.schema_type == "builder":
        schema = json.loads(schema) if isinstance(schema, str) else {"name": schema}
        ...
        schema = AutoTransformSchema.from_data(json.loads(os.getenv(schema, "")))
    ...
    runner = get_runner(args, event_handler)
...
def get_runner(args: Namespace, event_handler: EventHandler) -> Runner:
    """Get the runner based on the arguments.
