...
def test_all_enum_values_present():
    ...
    missing_values = [
        input_type for input_type in InputName if input_type not in FACTORY.get_components()
    ]
    assert not missing_values, "Types missing from factory: " + ", ".join(missing_values)

    extra_values = [
        input_type for input_type in FACTORY.get_components() if input_type not in InputName
    ]
...
def test_encoding_and_decoding():
    ...
    for name, components in test_components.items():
        ...
        for component in components:
            assert component.name == name, f"Testing input of type {component.name} for type {name}"
            assert (
