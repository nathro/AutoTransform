...
"""Tests that the EventNotifier's factory is correctly setup."""

from typing import Any, Dict, List

from autotransform.event.notifier.base import FACTORY, EventNotifierName
...
def test_all_enum_values_present():
    ...
    missing_values = [
        event_notifier_name
        for event_notifier_name in EventNotifierName
        if event_notifier_name not in FACTORY.get_components()
    ]
    assert not missing_values, "Names missing from factory: " + ", ".join(missing_values)

    extra_values = [
        event_notifier_name
        for event_notifier_name in FACTORY.get_components()
        if event_notifier_name not in EventNotifierName
    ]
...
def test_fetching_and_bundling() -> None:
    ...
    test_components: Dict[EventNotifierName, List[Dict[str, Any]]] = {
        EventNotifierName.CONSOLE: [{}],
    }

    for name in EventNotifierName:
        assert name in test_components, f"No test components for EventNotifier {name}"

    for name, components in test_components.items():
        assert name in EventNotifierName, f"{name} is not a valid EventNotifierName"
        for component in components:
            ...
            ), f"Testing EventNotifier of name {component_instance.name} for name {name}"
