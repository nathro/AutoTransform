...
def test_all_enum_values_present():
    ...
    missing_values = [step_name for step_name in StepName if step_name not in factory_components]
    assert not missing_values, f"Names missing from factory: {', '.join(missing_values)}"

    extra_values = [step_name for step_name in factory_components if step_name not in StepName]
